...
class SignalRClient:
    ...
    async def _on_invocation_message(self, message: InvocationMessage) -> None:
        ...
        if not callbacks:
            ...
            if expects_response:
                _logger.error("No result given for '%s' method and invocation ID '%s'.", message.target, message.invocation_id)
            ...
            try:
                ...
                if res:
                    if expects_response:
                        ...
                        _logger.warning("Result given for '%s' method but server is not expecting a result.", message.target)
